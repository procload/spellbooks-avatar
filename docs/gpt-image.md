...
This API supports `gpt-image-1`, `gpt-image-1-mini`, as well as `dall-e-2` and `dall-e-3`.

> **Spellbooks Avatar note:** The avatar generator now defaults to OpenAI's `gpt-image-1-mini` model whenever `IMAGE_MODEL_PROVIDER=openai`. Adjust `IMAGE_MODEL_NAME` (or the provider-specific `OPENAI_IMAGE_MODEL`/`GEMINI_IMAGE_MODEL`) to switch models without code changes.
...
| GPT Image Mini | Image API: Generations, Edits | Lower latency and cost compared to `gpt-image-1`; ideal default for avatars |
